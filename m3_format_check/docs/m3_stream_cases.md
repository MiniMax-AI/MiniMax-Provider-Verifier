...
| 02_08 | `test_02_08_stream_no_utf8_replacement_char_in_emoji` | 使用高密度 emoji 请求连续执行 20 次，检查任意 `delta.content` 中都不出现 `U+FFFD`（`�`）替换字符 | 防止 provider 把四字节 emoji 从 UTF-8 分帧中间截断（0823 innomatrix emoji 截断问题） |
