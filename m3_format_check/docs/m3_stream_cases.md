...
- 本组 case 只关注分包质量，不校验正文长度、JSON 合法性、工具名称、参数内容、工具数量等功能正确性。
...
| tool：作文写文件 | 15% | 85% | 0% | 95% | 0% |
| tool：500 字符 string | 10% | 90% | 0% | 95% | 0% |
...
| stream-02 | `essay_500_chars_to_temp_file` | 通过 `write_file` 写入约 500 字作文 | `arguments` 分包 |
| stream-03 | `structured_json_1k` | 返回不少于 1KB 的结构化 JSON，默认执行 5 次 | 合并 5 次 `content` 分包后判定 |
| stream-04 | `tool_string_500_chars` | 单工具约 500 字符 string 参数 | `arguments` 分包 |
| stream-05 | `tool_string_10k_chars` | 单工具约 10K 字符 string 参数，标记为 `slow` | `arguments` 分包 |
...
| stream-08 | `reasoning_then_tool_call` | adaptive reasoning 后调用写文件工具 | `arguments` 分包 |
